...
Font.register({
  ...
  fonts: [
    ...
    {
      src: "/fonts/Fraunces-SemiBold.ttf",
      fontWeight: 600,
      fontStyle: "normal",
    },
...
function buildStyles(tpl: DocTemplate, font?: string) {
  ...
  return StyleSheet.create({
    ...
    wedgeBrandCol: { flexDirection: "column", alignItems: "flex-start" },
    wedgeLogo: { height: 44, maxWidth: 180, objectFit: "contain" },
    wedgeName: {
      fontSize: 13,
      fontWeight: 700,
      letterSpacing: 0.4,
    ...
    bulletDot: {
      width: 5,
      height: 5,
      backgroundColor: tpl.primary,
      marginTop: 3.5,
      marginRight: 8,
      transform: "rotate(45deg)",
    },
    bulletDesc: { flex: 1, fontSize: 10, lineHeight: 1.35, color: tpl.ink },
    bulletAmount: {
      fontSize: 10,
      fontWeight: 700,
      textAlign: "right",
    ...
    wedgeStampCol: { flexDirection: "column", alignItems: "flex-end" },
    wedgeStamp: {
      fontSize: 15,
      fontWeight: 700,
      textTransform: "uppercase",
    ...
    wedgeSocialsCol: { alignItems: "flex-end", marginTop: 6 },
    wedgeSocialLabelRow: { flexDirection: "row", alignItems: "center", marginTop: 3 },
    wedgeSocialLabel: { fontSize: 9, color: tpl.bandText, marginLeft: 2 },
    classicHeader: {
      alignItems: "center",
    ...
    sectionLabel: {
      ...
      color: tpl.titleAccent ? tpl.primary : tpl.ink,
      ...(tpl.titleAccent
        ? { borderLeftWidth: 3, borderLeftColor: tpl.primary }
        : {}),
      textTransform: "uppercase",
    },
    itemRow: {
...
function FlowerPattern({ color }: { color: string }) {
  ...
  for (
    let y = FLOWER_SPACING / 2;
    y < PAGE_H - FLOWER_SPACING / 2;
    y += FLOWER_SPACING
  ) {
    for (
      let x = FLOWER_SPACING / 2;
      x < PAGE_W - FLOWER_SPACING / 2;
      x += FLOWER_SPACING
    ) {
...
export function LetterPdf({
  ...
  return (
    <Document
      title={`${doc.title} — ${client?.name ?? doc.id}`}
      author={company.name}
    >
        ...
        <View style={s.pageInner}>
          ...
          {tpl.layout === "classic" ? (
            <View style={s.classicHeader}>
              ...
              {contactLine ? (
                <Text style={s.classicMeta}>{contactLine}</Text>
              ) : null}
              {company.regNo ? (
                <Text style={s.classicMeta}>RC {company.regNo}</Text>
              ) : null}
              <View style={s.classicRule} />
              <Text
                style={[
                  s.classicDate,
                  { textAlign: "center", alignSelf: "stretch", marginTop: 8 },
                ]}
              >
            ...
            <View style={s.bandWrap}>
              <View style={s.bannerHeader}>
                ...
                {contactLine ? (
                  <Text style={s.bannerMeta}>{contactLine}</Text>
                ) : null}
                {company.regNo ? (
                  <Text style={s.bannerMeta}>RC {company.regNo}</Text>
                ) : null}
                <View style={[s.bannerMetaRow, { justifyContent: "center" }]}>
              ...
              {tpl.wave ? (
                <Svg
                  style={s.headerWaveSvg}
                  viewBox={`0 0 ${PAGE_W} 34`}
            ...
            <View style={s.wedgeHeader}>
              <Svg
                style={s.wedgeHeaderWave}
                viewBox={`0 0 ${PAGE_W} 96`}
                preserveAspectRatio="none"
              >
                <Path d={wedgeDiagonal(PAGE_W, 96)} fill={tpl.accentStrip} />
              </Svg>
              <View style={s.wedgeBrandCol}>
                {company.logo ? (
                  // eslint-disable-next-line jsx-a11y/alt-text
                  <Image src={company.logo} style={s.wedgeLogo} />
                ) : null}
                {company.tagline ? (
                  <Text style={s.wedgeTagline}>{company.tagline}</Text>
                ) : null}
                {doc.showCompanyDetails ? (
                  <>
                    <Text style={s.wedgeName}>{company.name}</Text>
                    {contactLine ? (
                      <Text style={s.wedgeMeta}>{contactLine}</Text>
                    ) : null}
                    {company.regNo ? (
                      <Text style={s.wedgeMeta}>RC {company.regNo}</Text>
                    ) : null}
                  </>
                ) : null}
              </View>
              <Text style={s.wedgeDate}>{formatLetterDate(doc.date)}</Text>
            </View>
          ) : (
            <>
          ...
          <View
            style={[
              ...
              ...(!tpl.band
                ? [{ paddingTop: tpl.layout === "classic" ? 20 : 34 }]
                : []),
              ...(tpl.layout === "banner" ? [{ paddingTop: 36 }] : []),
              ...(tpl.layout === "wedge" ? [{ paddingTop: 14 }] : []),
            ]}
          >
            <View style={s.content}>
              ...
              <View style={s.titleWrap}>
                {tpl.layout === "wedge" ? (
                  <Text style={s.titleUnderline}>{doc.title}</Text>
                ) : (
                  <>
                    ...
                    {section.items.map((item) =>
                      tpl.layout === "wedge" ? (
                        <View key={item.id} style={s.bulletRow}>
                          <View style={s.bulletDescWrap}>
                            <View style={s.bulletDot} />
                            <Text style={s.bulletDesc}>{item.description}</Text>
                          </View>
                          <Text style={s.bulletAmount}>
                ...
                {doc.showTotal ? (
                  tpl.layout === "wedge" ? (
                    <View style={s.bulletRow}>
                      ...
                      <Text style={s.bulletTotalAmount}>
                        {formatNaira(total)}
                      </Text>
                    </View>
                  ) : (
                    <View
                      style={tpl.totalChip ? s.totalRowChip : s.totalRowPlain}
                    >
          ...
          {hasFooter ? (
            tpl.layout === "wedge" ? (
              <View style={s.wedgeFooter}>
                <Svg
                  style={s.wedgeFooterWave}
                  viewBox={`0 0 ${PAGE_W} 100`}
                  preserveAspectRatio="none"
                >
                  <Path d={wedgeDiagonal(PAGE_W, 100)} fill={tpl.accentStrip} />
                </Svg>
                <View>
                  {company.phone ? (
                    <View style={s.wedgeContactRow}>
                      <PdfSocialIcon
                        icon={CONTACT_ICONS.phone}
                        color={tpl.bandText}
                        size={9}
                      />
                      <Text style={s.wedgeContactText}>{company.phone}</Text>
                    </View>
                  ) : null}
                  {company.email ? (
                    <View style={s.wedgeContactRow}>
                      <PdfSocialIcon
                        ...
                        size={9}
                      />
                      <Text style={s.wedgeContactText}>{company.email}</Text>
                    </View>
                  ) : null}
                  {company.website ? (
                    <View style={s.wedgeContactRow}>
                      <PdfSocialIcon
                        ...
                        size={9}
                      />
                      <Text style={s.wedgeContactText}>{company.website}</Text>
                    </View>
                  ) : null}
                  {company.address ? (
                    <View style={s.wedgeContactRowStart}>
                      <PdfSocialIcon
                        icon={CONTACT_ICONS.pin}
                        color={tpl.bandText}
                        size={9}
                      />
                      <Text style={s.wedgeContactText}>{company.address}</Text>
                    </View>
                  ) : null}
                </View>
                <View style={s.wedgeStampCol}>
                  <Text style={s.wedgeStamp}>{company.name}</Text>
                  {wedgeSocials.length ? (
                    <View style={s.wedgeSocialsCol}>
                      {wedgeSocials.map((entry) => (
                        <View key={entry.key} style={s.wedgeSocialLabelRow} wrap={false}>
                          <PdfSocialIcon icon={SOCIAL_ICONS[entry.key]} color={tpl.bandText} size={10} />
                          <Text style={s.wedgeSocialLabel}>{entry.value}</Text>
                        </View>
                      ))}
                    </View>
                  ) : null}
              ...
              <View style={tpl.band ? s.footerBand : s.footerPlain}>
                <Text style={tpl.band ? s.footerContact : s.footerContactPlain}>
                  {contactLine}
                </Text>
                {socials.length ? (
                  <View style={s.socialsRow}>
                    {socials.map((entry) => (
                      <View key={entry.key} style={s.socialItem} wrap={false}>
                        <PdfSocialIcon
                          icon={SOCIAL_ICONS[entry.key]}
                          color={tpl.band ? tpl.bandMuted : tpl.faint}
                          size={10}
                        />
                        <Text
                          style={
                            tpl.band ? s.footerSocials : s.footerSocialsPlain
                          }
                        >
                          {entry.value}
                        </Text>
