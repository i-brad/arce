...
async function check(url, expectedTexts, label) {
  ...
  console.log(
    `[${label}] ${missing.length ? "MISSING: " + missing.join(" | ") : "OK"}`,
  );
}
...
await check(
  "/",
  ["Recent documents", "No documents yet.", "No clients yet."],
  "dashboard (empty)",
);

await check(
  "/documents",
  ["No documents yet.", "New document"],
  "documents list (empty)",
);
await check(
  "/clients",
  ["No clients yet.", "Add client"],
  "clients list (empty)",
);
await check(
  "/settings",
...
if (companyInput !== "Casa khanya Homes") failures++;
console.log(
  `[settings] company input: ${companyInput === "Casa khanya Homes" ? "OK" : "FAIL"}`,
);
...
if (amountWidth > 260) {
  failures++;
  console.log(
    `[editor] amount field width: ${amountWidth}px (should be ~144px)`,
  );
} else {
...
console.log(
  "[settings save] " +
    (settingsBody.includes("Replace logo") ? "OK" : "FAIL (logo not saved)"),
);
...
console.log(
  "[settings] " +
    (patternSettingsBody.includes("Replace pattern")
      ? "pattern upload: OK"
      : "FAIL (pattern not saved)"),
);
...
console.log(
  "[logo+socials on doc] " +
    (missDoc.length ? "MISSING: " + missDoc.join(" | ") : "OK"),
);
...
console.log(
  "[footer socials+contact] " +
    (missFooter.length ? "MISSING: " + missFooter.join(" | ") : "OK"),
);
...
console.log(
  `[pdf download] ${dlPath ? "OK (" + download.suggestedFilename() + ")" : "FAILED"}`,
);
if (!dlPath) failures++;
const pdfBuf = await import("node:fs/promises").then((fs) =>
  fs.readFile(dlPath),
);
...
const printState = await page.evaluate(() => {
  ...
  return {
    printOnlyDisplay: printOnly
      ? getComputedStyle(printOnly).display
      : "missing",
    printOnlyVisible: printOnly
      ? getComputedStyle(printOnly).visibility
      : "missing",
...
if (!newDocBody.includes("TOTAL")) {
  failures++;
  console.log(
    "[defaults] new document: TOTAL missing (showTotal should default on)",
  );
} else {
...
if (desktopSidebarHidden === "none") {
  ...
  failures++;
  console.log(
    `[mobile] desktop sidebar should be hidden, got ${desktopSidebarHidden}`,
  );
}
